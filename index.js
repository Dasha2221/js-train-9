...
function findElementGreaterThan(arr, num) {
  ...
  if (foundElement !== undefined) {
    ...
    // Якщо не знайдено елемента, що задовільняє умову, повертаємо рядок
...
function getPositiveArrayValues(arr) {
  ...
  const iterArray = Array.from(iterValues);
  const result = iterArray.filter((value) => value > 0);
  return result;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function removeAndDouble(arr, startIdx) {
  ...
  const doubledArray = removeAnd.map((element) => element * 2);
  return doubledArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function sumAndPrint(arr) {
  ...
  // Виводимо кожен елемент масиву в консоль за допомогою методу forEach()
  arr.forEach((element) => {
    console.log(element);
  });
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  // Вирівнюємо масив за допомогою методу flat()
  const flattenedArray = arr.flat();

  // Підносимо кожен елемент до квадрату за допомогою методу map()
  const squaredArray = flattenedArray.map((element) => element ** 2);

  // Фільтруємо елементи за допомогою методу filter(), які більше за порогове значення
